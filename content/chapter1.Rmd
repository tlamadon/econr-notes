...
First we called `options(mpeccable.coloring=TRUE)` to tell `mpeccable` that we don't want th actual values of the Jacobian, but we just want to know what parts are potentially non zero. The righ of the figure contains a diagonal matrix. This represents the derivative of $latex V $ with respect to the control variables $a^*_j$. There is exactly one control per state $(a_j,z_j)$.
